      function [XD,Y]=f16(T,X,U,constants);
...
%                     Copyright Jan. 1992  Brian L Stevens
      ...
      VT=X(1);
      ALPHA=X(2)*constants.RTOD;
      BETA =X(3)*constants.RTOD;
      PHI  = X(4);
      THETA= X(5);
      PSI  = X(6);
      P    = X(7);
...
%
      PHID  = PHI*constants.RTOD;
      THETAD= THETA*constants.RTOD;
      PD    = P*constants.RTOD;
      QD    = Q*constants.RTOD;
      RD    = R*constants.RTOD;
%
...
%
      ...
      CMT=  CMT + CQ  *   D(7)        +       CZT * (constants.XCGR-constants.XCG);
      CNT=  CNT + B2V * ( D(8)*R + D(9)*P ) - CYT * (constants.XCGR-constants.XCG) * constants.CBAR/constants.B;
...
%
      ...
      CPSI  = cos(PSI);
%
      RM = (constants.G/constants.WEIGHT);
      QS    = QBAR * constants.S;
      QSB   = QS * constants.B;
      RMQS  = RM * QS;
      GCTH  = constants.G * CTH;
      QSPH  = Q * SPH;
      AX    = RM * (QS * CXT + T);
      AY    = RMQS * CYT;
      AZ    = RMQS * CZT;
...
%  STATE EQUATIONS
%  FORCES.
      UDOT  =  R*V - Q*W - constants.G*STH    +   AX;
...
%
%  MOMENTS.
      XD(7) =  (constants.C2*P + constants.C1*R + constants.C4*constants.HE)*Q + QSB*(constants.C3*CLT + constants.C4*CNT);
      XD(8) =  (constants.C5*P - constants.C7*constants.HE)*R + constants.C6*(R*R-P*P) +QS*constants.CBAR*constants.C7*CMT;
      XD(9) =  (constants.C8*P-constants.C2*R+constants.C9*constants.HE)*Q + QSB*(constants.C4*CLT + constants.C9*CNT);
%
%  NAVIGATION
      T1= SPH * CPSI;
      T2= CPH * STH;
...
%
%  OUTPUTS :
%Measured acclerometer (AZ at pilot location) in G's
      AZN     =  (-AZ + constants.XA * XD(8) )/constants.G;
%AX and AY at CG, in G's
      AYN   =   AY/constants.G;
      AXN   =   AX/constants.G;
%CSTAR PARAMETER      
      CSTAR= AZN + 12.4*Q;
